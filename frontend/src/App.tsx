...
function Shell() {
  ...
  const showInitialLoadingScreen = isInitializing
  ...
  return (
    <main className="relative h-[100dvh] min-h-0 overflow-hidden bg-transparent font-body text-ink-800">
      <LoadingScreen isVisible={showInitialLoadingScreen} variant="initial" />
      <div className="absolute -left-24 top-10 h-64 w-64 rounded-full bg-sage-100/38 blur-3xl" />
      <div className="absolute -right-16 bottom-16 h-72 w-72 rounded-full bg-clay-200/28 blur-3xl" />
      <div
      ...
      {isChatRoute && isInsightsRailOpen ? (
        <div className="absolute inset-y-0 right-[20rem] hidden w-px bg-clay-200/60 lg:block" />
      ) : null}
      ...
      <div className="relative z-10 flex h-full min-h-0">
        ...
        <section className="flex h-full min-h-0 flex-1 flex-col bg-sand-50/60">
          {error ? (
            <div className="px-4 pt-4 sm:px-6">
              <p className="rounded-soft border border-clay-200 bg-clay-100 px-4 py-3 text-sm text-ink-800">{error}</p>
            </div>
          ) : null}
          ...
          <div className="flex min-h-0 flex-1">
            ...
            {isChatRoute && isInsightsRailOpen ? (
              <ChatInsightsRail className="hidden lg:block lg:w-[20rem]" onClose={() => setIsInsightsRailOpen(false)} />
            ) : null}
          </div>
        </section>
        ...
        {isChatRoute && isInsightsOpen ? (
          <div className="fixed inset-0 z-40 flex justify-end lg:hidden" role="dialog" aria-modal="true">
            ...
            />
            <ChatInsightsRail className="w-[86%] max-w-[344px]" onClose={() => setIsInsightsOpen(false)} />
          </div>
        ) : null}
      </div>
